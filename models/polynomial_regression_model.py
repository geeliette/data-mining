# polynomial_regression_speed_profile.py
import pandas as pd
import numpy as np
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import PolynomialFeatures
from sklearn.linear_model import LinearRegression
from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score

# Load data
data = pd.read_csv("trip_speed_profile_data.csv")
print(f"Loaded {len(data)} trips\n")

# Select features
target = "fuel_consumed_ml"
ignore_cols = ["file", target]
features = [c for c in data.columns if c not in ignore_cols]

print("Using features:")
print(features, "\n")

X = data[features].values
y = data[target].values

# Polynomial expansion (degree 2)
poly = PolynomialFeatures(degree=2, include_bias=False)
X_poly = poly.fit_transform(X)
feature_names = poly.get_feature_names_out(features)

# Train/test split
test_size = 0.2 if len(data) >= 200 else 0.3
X_train, X_test, y_train, y_test = train_test_split(X_poly, y, test_size=test_size, random_state=42)

# Train model
lr = LinearRegression()
lr.fit(X_train, y_train)
y_pred = lr.predict(X_test)

# Evaluate
mae = mean_absolute_error(y_test, y_pred)
rmse = np.sqrt(mean_squared_error(y_test, y_pred))
r2 = r2_score(y_test, y_pred)

print("="*70)
print("Model B: Polynomial Regression (Degree 2)")
print("="*70)
print(f"Samples: {len(data)} trips")
print(f"Expanded features: {len(feature_names)}\n")

print("Top 10 Coefficients:")
coef_df = pd.DataFrame({"Feature": feature_names, "Coefficient": lr.coef_})
coef_df["abs_val"] = coef_df["Coefficient"].abs()
coef_df = coef_df.sort_values("abs_val", ascending=False).drop(columns="abs_val").head(10)
print(coef_df.to_string(index=False))

print("\nPerformance:")
print(f"  MAE:  {mae:.2f} ml")
print(f"  RMSE: {rmse:.2f} ml")
print(f"  R²:   {r2:.4f}")